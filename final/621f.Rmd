---
title: "Currency Valuation in Myanmar"
author: "Sam Reeves"
output: pdf_document
---
...
Predicting timeseries data from its own history is a losing maneuver.  For clear theoretical reasons, it shouldn't work.  We identify one example where this method proves very useful and accurate.  Because the Central Bank of Myanmar cannot effectively control its money supply or its balance of imports, it fixes exchange rates in a way that can be revealed by multiple regression models.
...
The Central Bank of Myanmar declares an exchange rate every morning for each of 38 currencies which can be used to buy Kyats.  For nearly 10 years of data, each day is an observation, and there are 38 variables in each observation which are all numerical exchange rates.

### Data collection 

This data was scraped from the Central Bank of Myanmar's main website.  This dataset is inconsistently available and is subject to change, however, we are sure that we have compiled a dataset which represents real information for its full period.  We saved this data to disk and filled any missing days with the previous day's value.  We make the assumption that trading still happened on days that did not have a new rate.
